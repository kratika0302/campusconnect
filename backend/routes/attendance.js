...
// Get attendance for a course on a date (faculty) — FIXED param order bug
router.get('/course/:courseId', auth(['faculty', 'admin']), async (req, res) => {
  try {
    const { date } = req.query;
    const params = [];

    // Build LEFT JOIN condition - date filter goes in JOIN not WHERE
    let joinOn = 'a.student_id = s.id AND a.course_id = e.course_id';
    if (date) {
      joinOn += ' AND a.date = ?';
      params.push(date);
    }
